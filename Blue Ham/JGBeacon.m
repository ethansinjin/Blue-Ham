...
-(id)init{
    if (self = [super init]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reconnectBluetoothPeripheral) name:@"reblue" object:nil];
    }
    return self;
}
...
-(void)terminate {
    [self.receiveBeacon terminate];
}

-(void)reconnectBluetoothPeripheral {
    [self.receiveBeacon reconnectBluetoothPeripheral];
}
-(BOOL)advertising{
